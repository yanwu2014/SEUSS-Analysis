...
coefs.matrix.endo <- unflatten.dataframe(coefs.df.endo, "cf", row.col = "Gene", col.col = "Group")
coefs.matrix.endo <- coefs.matrix.endo[, sig.genotypes.endo]

coefs.matrix.multi <- unflatten.dataframe(coefs.df.multi, "cf", row.col = "Gene", col.col = "Group")
...
gene_clusters <- flatten.genotypes.list(gene_clusters_list)

## GO enrichment
genesets <- load.genesets("go_bp_genesets.gmt")

bg.genes <- unique(unlist(gene_clusters_list, F, F))
genesets <- filter.genesets(genesets, bg.genes, min.size = 30, max.size = 500)
names(genesets) <- sapply(names(genesets), function(x) gsub("GO_", "", x))

gene.clusters.enrich <- multiple.fisher.enrich(gene_clusters_list, genesets)
...
tfs.modules.matrices.list <- lapply(coefs.matrices.list, function(X) {
  calc.gene_modules.effect(X, gene_clusters_list, gene_module_mapping, min.coef = 0.025)
})
...
pdf("tf_gene_module_effect.pdf", width = 15, height = 4.5)
