...
n_cells_counts_list <- lapply(genotype_files, function(x) sapply(read.genotypes(x), length))
...
ggcorrelation(n_cells_logfc[,"Stem cell"], n_reads_logfc[rownames(n_cells_logfc), "Stem cell"], 
              ...
              points.label = genes.lbl, use.label = T, box = T, show.corr = T)
dev.off()
...
ggcorrelation(n_diff_genes, log.fc.use, NULL, NULL, show.corr = F, box = F, use.label = T, points.label = pts.label,
